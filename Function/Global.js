...
let { MessageEmbed, GuildMember, Message } = require('discord.js');
...
Promise.prototype.Delete = function(time) {
  setTimeout(() => {
     if (this) this.then(s => {
      if (s.deletable) s.delete().catch(e => {});
  });
  }, time * 1000)
};
Message.prototype.React = function(emj) {
  if(!this) return;
  let Emj = Client.SearchEmojis(emj) || emj;
  this.react(Emj).catch(() => {});
};
...
GuildMember.prototype.nameCheck = function () {
 /* if(params === "analiz") {
    let Name = this.guild.members.cache.filter(global.Config.Tags.Other.some(Tag => this.user.username.toLowerCase().includes(Tag.toLowerCase()))).size
  }*/
  //if (!this.manageable) return false;
  
  if(global.Config.Tags.Other.some(Tag => this.user.username.includes(Tag)) || this.user.discriminator == global.Config.Tags.Number) return true; else return false
};
GuildMember.prototype.Permissions = function (array) {
  if (array && Array.isArray(array) == false) array = [array];
  if (global.Config.Bot.Owner.some((ID) => this.roles.cache.has(ID) || this.id === ID) || this.guild.ownerID === this.id || this.permissions.has("ADMINISTRATOR") || (array && array.length && (array).concat(global.Config.Roles.Authorized.Management).some((ID) => this.roles.cache.has(ID) || this.id === ID))) return true;
  else return false;
};
...
Client.SearchEmojis = function (target) {
  let Output = Client.emojis.cache.get(target) || Client.emojis.cache.find(x => x.name === target || x === target) || " "
  return Output;
};
Client.SearchChannels = function (target) {
  let Output = Client.channels.cache.get(target) || Client.channels.cache.find(x => x.name === target || x === target) || undefined;
  return Output;
}
Client.Time = function (value) {
    ...
    return (days ? days + ' G. ' : '') + (hours ? hours + ' s. ' : '') + (minutes ? minutes + ' dkk. ' : '') + (seconds ? seconds + ' sn.' : '')
};
Client.StatsTime = function (value) {
  if(value === 0) return "0 dakika"
    const hours = Math.floor(value / 3600000);
    value = value % 3600000;
    const minutes = Math.floor(value / 60000);
    value = value % 60000;
    const seconds = Math.floor(value / 1000);
    return (hours ? hours + ' s. ' : '') + (minutes ? minutes + ' dkk. ' : '') + (hours ? '' : seconds ? seconds + ' sn.' : '')
};
Client.MsgCheck = function(message) {
  ...
  if(Link.test(message.content)) {
      message.channel.send({ embeds: [new MessageEmbed({ 
        color: "RED",
        description: "İçerik __link__ veya __URL__ içerdiği için yayınlanamıyor."
      })] }).Delete(3) 
  }
  if(Küfür.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(message.content))) {
    message.channel.send({ embeds: [new MessageEmbed({ 
      color: "RED",
      description: "İçerik __küfür__, __hakaret__ veya __argo__ içerdiği için yayınlanamıyor."
    })] }).Delete(3)
    return
}
}
Client.sleep = function (delay) {
  var start = new Date().getTime();
  while (new Date().getTime() < start + delay);
}
Client.RolesPermissionConverter = function (Array) {
  let Permissions = {
    ...
    "READ_MESSAGE_HISTORY": "Mesaj Geçmişi Oku",
    "READ_MESSAGE_HISTORY": "Tüm Rollerden ve Ever&Here Bahset",
    "USE_EXTERNAL_EMOJIS": "Harici Emoji Kullan",
    "VIEW_GUILD_INSIGHTS": "Sunucu Bilgilerini Görüntüle",
    "CONNECT": "Bağlan",
    "SPEAK": "Konuş",
    "MUTE_MEMBERS": "Susturma",
    "DEAFEN_MEMBERS": "Sağırlaştırma",
    "MOVE_MEMBERS": "Üyeleri Taşı",
    "USE_VAD": "Ses Eylemini Kullan",
    "CHANGE_NICKNAME": "Kullanıcı Adunı Değiştir",
    ...
    "REQUEST_TO_SPEAK": "Konuşma Talebi",
    "MANAGE_EVENTS": "Etkinlikleri Yönet",
    "MANAGE_THREADS": "Alt Başlıkları Yönet",
    "CREATE_PUBLIC_THREADS": "Genel Alt Başlık Oluştur",
    "CREATE_PRIVATE_THREADS": "Özel Alt Başlık Oluştur",
    "USE_EXTERNAL_STICKERS": "Harici Çıkartma Kullan",
    "SEND_MESSAGES_IN_THREADS": "Altbaşlıklara Mesaj Gönder",
    "USE_EMBEDDED_ACTIVITIES": "Kullanıcı Etkinlikleri",
    "MODERATE_MEMBERS": "Zaman Aşımı Uygula",
  }
  let Arr = [];
  for (let i = 0; i < Array.length; i++) {
    const element = Array[i];
    Arr.push(Permissions[element]);
  };
  return Arr;
}