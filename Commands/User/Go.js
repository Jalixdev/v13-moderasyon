...
module.exports = {
  name: "go",
  description: "Kullanıcının yanına gidersiniz.",
  category: "User",
  aliases: ["git", "götür"], 
  run: async (client, message, args, EmojiData, LogData, Config, RolData) => {
    ...
    let Msg = await message.channel.send({content: `${Member}` , embeds: [Emb.setDescription(`Selam \`${Member.displayName}\`, ${message.author} kullanıcısı sizin bulunduğunuz <#${message.member.voice.channel.id}> kanalına girmek istiyor.(Kalan Süre: <t:${Math.floor((Date.now()+1000*30) / 1000)}:R>)`)], components: [new MessageActionRow().addComponents(Onay, Yönetici, Ret)] });
